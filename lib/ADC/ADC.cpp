...
void ADC::init() {
  ...
  adcs[0] = ADS1015(I2C_ADDRESS_ADS1x15_0);
  adcs[1] = ADS1015(I2C_ADDRESS_ADS1x15_1);
  adcs[2] = ADS1015(I2C_ADDRESS_ADS1x15_2);
...
int16_t ADC::read(ADC::Pin port) {
  ...
  int16_t value = ADC::adcs[port >> 4].readADC(port & 0xf);
...
void read_adc_acceleration_recuperation(void *pvParameter) {
  ...
  while (1) {
    ...
    printf("v0=%6d, v1=%6d", value0, value1);
    ...
    if (accelChanged | recupChanged) {
      ...
      // write console log
      printf("Acceleration: %4d --> %4d | Recuperation:  %4d --> %4d | ACCEL-DISPLAY: %d\n", value0, accel, value1, recup, accDisplay);
