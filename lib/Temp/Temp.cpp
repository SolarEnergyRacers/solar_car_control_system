...
int get_num_temp_dev(void){
    return ds.getDeviceCount(); // no mutex required, since it does not actually communicate with the sensor
}
...
float read_tempC_index(int index){
    ...
    return ds.getTempCByIndex(index); // no mutex required, since it does not actually communicate with the sensor
}
...
float read_tempF_index(int index){
    ...
    return ds.getTempFByIndex(index); // no mutex required, since it does not actually communicate with the sensor
}
